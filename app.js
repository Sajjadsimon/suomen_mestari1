...
(function() {
  ...
  function speakFinnish(text) {
    ...
    // Clean word of parentheticals or slashes for natural pronunciation
    ...
    const fiVoice = voices.find(v => v.lang.startsWith('fi') || v.lang.startsWith('FI'));
  ...
  function updateTodayStats() {
    ...
    document.getElementById('stat-new-count').textContent = Math.min(newCount, 15);
  ...
  function startTodaySession() {
    ...
    currentQueue = [...dueList, ...newList.slice(0, 15)];
    if (currentQueue.length === 0) {
      // Fallback: pick any 10 words from selection for extra practice
  ...
  function renderCurrentCard() {
    if (!currentQueue || currentQueue.length === 0) {
      document.getElementById('card-front-main').textContent = 'Kaikki kerrattu!';
      document.getElementById('card-front-hint').textContent = 'Hienoa työtä! Palaa huomenna tai valitse toinen kappale.';
    ...
    document.getElementById('card-front-pos').textContent = concept.part_of_speech || 'sanasto';
    ...
    if (currentMode === 'fi_to_en') {
      ...
      document.getElementById('card-front-hint').textContent = 'Napauta kääntääksesi kortti';
    ...
    if (concept.forms && concept.forms.length > 0) {
      formsEl.innerHTML = `Muodot: <strong>${concept.forms.map(f => f.form).join(', ')}</strong>`;
      formsEl.style.display = 'block';
    } else {
      formsEl.style.display = 'none';
    }

    // Confusing pairs detection
    ...
    if (vocabData && vocabData.relations) {
      ...
      if (rels.length > 0) {
        const otherId = rels[0].from_id === concept.id ? rels[0].to_id : rels[0].from_id;
        const otherConcept = conceptsMap.get(otherId);
        if (otherConcept) {
          confusingEl.innerHTML = `⚠️ Huom! Älä sekoita sanaan: <strong>${otherConcept.finnish}</strong> (${otherConcept.english})`;
  ...
  function handleSRSResult(result) {
    ...
    if (currentIndex < currentQueue.length) {
      ...
      renderCurrentCard(); // shows completion card
      updateTodayStats();
    }
  }
  ...
  function checkDiacritics(typed) {
    ...
    if (target.includes('ä') && cleanTyped.includes('a') && !cleanTyped.includes('ä')) {
      feedbackEl.textContent = '💡 Vinkki: Tässä sanassa on etuvokaali Ä (ei A)!';
      feedbackEl.style.display = 'flex';
    } else if (target.includes('ö') && cleanTyped.includes('o') && !cleanTyped.includes('ö')) {
      feedbackEl.textContent = '💡 Vinkki: Tässä sanassa on etuvokaali Ö (ei O)!';
      feedbackEl.style.display = 'flex';
    } else {
      feedbackEl.style.display = 'none';
    }
  ...
  function openWordModal(concept) {
    ...
    if (vocabData && vocabData.relations) {
      const rels = vocabData.relations.filter(r => (r.from_id === concept.id || r.to_id === concept.id) && (r.type === 'commonly_confused' || r.type === 'looks_similar'));
      if (rels.length > 0) {
        confBox.style.display = 'block';
        confContent.innerHTML = '';
        rels.slice(0, 3).forEach(r => {
          const oid = r.from_id === concept.id ? r.to_id : r.from_id;
          const other = conceptsMap.get(oid);
          if (other) {
            const div = document.createElement('div');
            div.style.cssText = 'padding: 4px 8px; background: rgba(255,255,255,0.05); border-radius: 6px; font-size: 0.85rem;';
            div.innerHTML = `<strong>${other.finnish}</strong> = ${other.english}`;
      ...
      reader.onload = (event) => {
        try {
          ...
          if (parsed.learning_states) {
            ...
            alert('Edistymistiedot tuotu onnistuneesti!');
            updateTodayStats();
            renderStatsView();
            updateStreakDisplay();
          } else {
            alert('Virheellinen tiedosto: oppimistietoja ei löytynyt.');
          }
        } catch {
